...
def get_laser_ref(segments, angle, xy_robot):
    ...
    max_dist=10000
    dist_theta = max_dist
    theta=np.float32(angle)+np.float32(xy_robot[2])
...
for i in range(particle_num):
    particle_pos.append([random.uniform(0,max_x),random.uniform(0,max_y),random.uniform(0,math.pi*2)])
...
def sample_kernel(rng_states,weight,old_particle_pos,particle_pos):
    ...
    if tt<0.05:
        ...
        particle_pos[ty][2]=xoroshiro128p_uniform_float32(rng_states,thread_id)*math.pi*2
    else:
        ...
        for i in range(len(weight)):
            if t-weight[i]<0:
                ...
                particle_pos[ty][2]=old_particle_pos[i][2]
                break
            else:
                t-=weight[i]
